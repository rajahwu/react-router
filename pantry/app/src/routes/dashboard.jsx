    ...
    >
      ...
      <ul style={{ listStyle: "none", padding: 0 }}>
        {pantries.length ? (
          pantries.map((pantry) => (
            <li key={pantry.id}>
              <Box sx={{ display: "flex", flexDirection: "column", alignItems: "start", mb: 2 }}>
                ...
                <Box sx={{ display: "flex" }}>
                <img src={pantry.imageUrl ?? "https://picsum.photos/100/100?random=1"} alt={pantry.imageAlt} style={{ width: "50px", height: "50px" }} />
                <Box>
                {pantry.items.length ? pantry.items.map((item) => <span key={item.id}>{item.name}</span>) : null}
                <Typography variant="body2">{pantry.items.length} items</Typography>
                </Box>
          ...
          <Button variant="contained" onClick={() => navigate('/pantries')}>+ Pantry</Button>
          </Box>
        )}
      </ul>
    </Box>
